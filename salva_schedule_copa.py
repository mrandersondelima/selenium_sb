...
class ChromeAuto():
            ...
            try:
                ...
                if jogo_encerrado['state'] == 'Won':
                    ...
                    with open('meta_ganho.txt', 'r') as f:
                        meta_ganho = float( f.read() )

                    meta_ganho = self.saldo * 0.001667 if not self.meta_fixa else meta_ganho
                    ...
                    if not self.meta_fixa:
                        with open('meta_ganho.txt', 'w') as f:
                            f.write(f'{(self.saldo*0.01667):.2f}')
    ...
    def salva_schedules(self):
        while True:
            conn = self.get_bd_connection()
            cur = conn.cursor()
            try:
                proximos_jogos = self.chrome.execute_script(f'let d = await fetch("https://sports.sportingbet.com/cds-api/bettingoffer/virtual/fixture-view?x-bwin-accessid=MjcxNjZlZTktOGZkNS00NWJjLTkzYzgtODNkNThkNzZhZDg2&lang=pt-br&country=BR&userCountry=BR&offerMapping=All&sportIds=101&competitionIds=100204&scheduleSize=10&forceFresh=1"); return await d.json();')
                proximo_jogo = proximos_jogos['fixture']
                
                time_casa = None
                time_fora = None

                if proximo_jogo['participants'][0]['properties']['type'] == 'HomeTeam':
                    time_casa = proximo_jogo['participants'][0]['name']['value']
                    time_fora = proximo_jogo['participants'][1]['name']['value']
                else:
                    time_casa = proximo_jogo['participants'][1]['name']['value']
                    time_fora = proximo_jogo['participants'][0]['name']['value']

                horario = proximo_jogo['startDate'][11:16]
                hora = int(horario.split(':')[0])
                minuto = int(horario.split(':')[1])
                now = datetime.today()  
                horario = datetime( now.year, now.month, now.day, hora, minuto, 0) - timedelta(hours=3)
                ...
                for market in proximo_jogo['optionMarkets']:
                    if market['name']['value'] == 'Resultado da partida' or market['name']['value'] == 'Match Result':
                        odd_empate = float( market['options'][1]['price']['odds'] )
                        odd_casa = float( market['options'][0]['price']['odds'] )
                        odd_fora = float( market['options'][2]['price']['odds'] )
                
                print(f'horario: {horario}\ntime_casa: {time_casa}\ntime_fora: {time_fora}\nodd_empate: {odd_empate}\nodd_casa: {odd_casa}\nodd_fora: {odd_fora}\n')
                cur.execute(f"insert into jogos_copa_mundo values ('{horario}', '{time_casa}', '{time_fora}', '{odd_empate}', '{odd_casa}', '{odd_fora}')")
                conn.commit()
                conn.close()
                sleep(10)
            except Exception as e:                
