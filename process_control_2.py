...
async def main():
    ...
    while True:
        try:
            ...
            try:
                last_time_check = le_de_arquivo('last_time_check_2.txt', 'string')    
            except:
                print('tentando matar o processo anterior')
                try:
                    Popen(f"taskkill /f /pid {proc.pid}")     
                    chrome_process_id = le_de_arquivo('chrome_process_id_2.txt', 'int')
            ...
            if not proc:
                proc = await asyncio.create_subprocess_exec("python", r"D:\anderson.morais\Documents\dev\sportingbet4\app_2.py",
