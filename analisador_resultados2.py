...
while True:
    p = Popen(['wsl', '/home/andersonmorais/monkeybet2/novo_results.sh'], stdout=PIPE, stdin=PIPE)
        ...
        if ultimo_id_lido != result_id and sem_ruido:
            ...
            if len(array_jogos) >= 2:
                ...
                if gols_jogo_1 == 2 and gols_jogo_2 == 2:
                    #telegram.envia_mensagem(f'{n_jogos_antes_padrao_2_2} ANTES DE PADRÃO 2 2 CHAMPIONS')
                    n_jogos_antes_padrao_2_2 = 0
            ...
            if len(array_jogos) >= 3:
                ...
                if gols_jogo_1 == 2 and gols_jogo_2 == 3 and gols_jogo_3 == 2:
                    #telegram.envia_mensagem(f'{n_jogos_antes_padrao_2_3_2} ANTES DE PADRÃO 2 3 2 CHAMPIONS')
                    n_jogos_antes_padrao_2_3_2 = 0
                # elif gols_jogo_1 == 2 and gols_jogo_2 == 2 and gols_jogo_3 == 2:
                #     telegram.envia_mensagem(f'{n_jogos_antes_padrao_2_2_2} ANTES DE PADRÃO 2 2 2 CHAMPIONS')
                #     n_jogos_antes_padrao_2_2_2 = 0
                # if gols_jogo_1 == 3 and gols_jogo_2 == 2 and gols_jogo_3 == 2:
                #     telegram.envia_mensagem(f'{n_jogos_antes_padrao_3_2_2} ANTES DE PADRÃO 3 2 2 CHAMPIONS')
                #     n_jogos_antes_padrao_3_2_2 = 0
                # elif gols_jogo_1 == 2 and gols_jogo_2 == 2 and gols_jogo_3 == 3:
                #     telegram.envia_mensagem(f'{n_jogos_antes_padrao_2_3_2} ANTES DE PADRÃO 2 2 3 CHAMPIONS')
                #     n_jogos_antes_padrao_2_2_3 = 0
                else:
                ...
                cur_1.execute(f"select * from padrao_tres_jogos_2 where padrao = '{padrao}'")
                cur_2.execute(f"select * from padrao_dois_jogos_2 where padrao = '{padrao_2}'")
                rows_1 = cur_1.fetchall()
                rows_2 = cur_2.fetchall()
                if len(rows_1) == 0:
                    cur_1.execute(f"insert into padrao_tres_jogos_2 values ('{padrao}', 1)")                    
                else:
                    total = rows_1[0][1]
                    cur_1.execute(f"update padrao_tres_jogos_2 set qt = {total + 1} where padrao = '{padrao}'")                    

                if len(rows_2) == 0:
                    cur_2.execute(f"insert into padrao_dois_jogos_2 values ('{padrao}', 1)")                    
                else:
                    total = rows_2[0][1]
                    cur_2.execute(f"update padrao_dois_jogos_2 set qt = {total + 1} where padrao = '{padrao}'")
            ...
            if soma_gols == 1:
                um_gol_seguido += 1
                if um_gol_seguido >= 4:
                    telegram.envia_mensagem(f'{um_gol_seguido} COM APENAS 1 GOL EM SEGUIDA. CHAMPIONS.')    
            else:
                um_gol_seguido = 0
